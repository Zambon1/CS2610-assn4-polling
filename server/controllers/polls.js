...
import { getAllPolls, getPollById, createPoll, getVotesForPoll, getOptionsForPoll, getVoteCountsForPollIds } from '../models/polls.js';
import { findById, findByIds } from '../models/users.js';
...
router.get('/', async (req, res) => {
    try {
        ...
        const creators = await findByIds(polls.map((p) => p.user_id));
        polls.forEach(poll => {
        ...
        const voteCount = await getVoteCountsForPollIds(polls.map(p => p.id));
        polls.forEach(poll => {
...
router.post('/', requireAuth, async (req, res) => {
    ...
    try {
        const poll = await createPoll(title, description, allowAnonymous, userId, options);
...
router.get('/:id', async (req, res) => {
    try {
        const poll = await getPollById(parseInt(req.params.id));
        if (!poll) {
        ...
        if (req.user) {
            const voteResult = await connection.query(
                "SELECT option_id FROM votes WHERE poll_id = $1 AND user_id = $2 LIMIT 1",
                [poll.id, req.user.id]
            );
            poll.has_voted = voteResult.rowCount > 0;
            poll.user_vote_option_id = voteResult.rows[0]?.option_id ?? null;
        } else {
        ...
        if (poll.user_id) {
            poll.creator = await findById(poll.user_id);
        }
...
router.post("/:id/vote", checkVoteAnonymity, async (req, res) => {
    ...
    // Logged-in users should always be tied to their account for one-vote enforcement.
    // Anonymous (not logged-in) voters use null user_id when poll allows anonymous voting.

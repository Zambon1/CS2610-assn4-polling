...
function PollDetails() {
  ...
  return (
    <section>
      ...
      <p>
        Created by {poll.creatorName} on {poll.createdAt ? new Date(poll.createdAt).toLocaleString() : "Unknown date"}
      </p>
      ...
      {canVote && (
        <form onSubmit={handleVoteSubmit}>
          ...
          {poll.options.map((option) => (
            <label key={option.id} style={{ display: "block", marginBottom: "0.5rem" }}>
              <input
                type="radio"
                name="selectedOption"
                value={option.id}
      ...
      {mustLoginToVote && !hasVoted && (
        <p className="info-message">
          You need to log in to vote on this poll. {" "}
          <Link to={`/login?redirect=/polls/${id}`}>Log in</Link>
        </p>
      )}
